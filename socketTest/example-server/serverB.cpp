	...
	while(1)
	{
		...
		for(i; i < desc.size(); i++) {
			//std::cout << "desc.seze = :" << desc.size() <<std::endl;
					...
					//messages0 = M2, messages1 = Hm1, messages2 = Base64(sign(Hm12)),messages3 = pubkey_string(client)
					string sign_Hm12;
					...
					messages[0] = DecodeRSAKeyFile("prikey2.pem",messages[0]);
					std::cout << "messages[0]:" << messages[0] << endl;
					string Hm2 = SHA1_hash(messages[0]);
					string Hm1 = messages[1];
					...
					int answer_verify =  VerifyRSAKeyFile("pubkey.pem",Hm12,sign_Hm12);  //verify the sign to affirm the validity of m1 and m2;
					...
					string answer;
					if(answer_verify)
						answer = "Server C verify success. your messages are:\n" + messages[0] + "\nthe virtual number is 12341234";
					else
