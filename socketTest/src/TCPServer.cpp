...
void TCPServer::SendFile(string file_name, int id)
{
	FILE *fp = fopen(file_name.c_str(),"r");
	std::cout << "start send file: " << file_name << std::endl;
        if(NULL == fp){
        ...
        while((length = fread(buffer,sizeof(char),4096,fp)) > 0){
                if(send(newsockfd[id]->socket,buffer,length,0) < 0)
                {
                        std::cout << "Send file failed:" << file_name <<std::endl;
                        break;
                }
                bzero(buffer,4096);
        }
	std::cout <<"Send file success:" << file_name << std::endl;
	fclose(fp);
